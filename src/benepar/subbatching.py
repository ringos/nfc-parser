...
def split(*data, costs, max_cost, force_even_size=False):
    ...
    """
    if force_even_size:
        # only count the costs of original examples
        all_data = data
        data = [[_ for i, _ in enumerate(items) if i%2==0] for items in data]
        costs = [_ for i, _ in enumerate(costs) if i%2==0]
    ...
    while costs_argsort:
        if subbatch_size == len(costs_argsort) or (
            ...
            if not force_even_size:
                subbatch_item_ids = costs_argsort[:subbatch_size]
                subbatch_data = [[items[i] for i in subbatch_item_ids] for items in data]
            else:
                # subbatch_item_ids = [costs_argsort[:subbatch_size][int(i/2)] for i in range(2*subbatch_size)]
                subbatch_item_ids = []
                for id in costs_argsort[:subbatch_size]:
                    subbatch_item_ids.extend([id*2, id*2 + 1])
                subbatch_data = [[items[i] for i in subbatch_item_ids] for items in all_data]
