...
def _get_left_labeled_spans(tree, left_spans_out, start):
    if isinstance(tree, str):
        return start + 1

    end = start
    for child in tree:
        end = _get_left_labeled_spans(child, left_spans_out, end)
    # Spans are returned as closed intervals on both ends

    if tree.left_sibling() is not None and not isinstance(tree.left_sibling(), str) and start != end-1:
        left_sibling = tree.left_sibling()
        left_spans_out.append((start, end - 1, left_sibling.label()))
    return end

def _get_right_labeled_spans(tree, right_spans_out, start):
    if isinstance(tree, str):
        return start + 1

    end = start
    for child in tree:
        end = _get_right_labeled_spans(child, right_spans_out, end)
    # Spans are returned as closed intervals on both ends

    if tree.right_sibling() is not None and not isinstance(tree.right_sibling(), str) and start != end-1:
        right_sibling = tree.right_sibling()
        right_spans_out.append((start, end - 1, right_sibling.label()))
    return end
...
def get_labeled_spans(tree):
    ...
    left_spans_out = []
    _get_left_labeled_spans(tree, left_spans_out, start=0)
    right_spans_out = []
    _get_right_labeled_spans(tree, right_spans_out, start=0)

    # [(0, 0, 'JJ'), (1, 1, 'NNS'), (0, 1, 'NP'), (2, 2, 'VBP'), (3, 3, 'VBN'), (4, 4, 'IN'), (5, 5, 'ADJ'), (6, 6, 'NNS'), (5, 6, 'NP'), (5, 6, 'NP'), (4, 6, 'PP'), (3, 6, 'VP'), (2, 6, 'VP'), (0, 6, 'S'), (7, 7, '.'), (0, 7, 'ROOT')]
    return [spans_out, left_spans_out, right_spans_out]
...
class ChartDecoder:
    ...
    def build_vocab(trees):
        label_set = set()
        for tree in trees:
            for _, _, label in get_labeled_spans(tree)[0]:
                if label:
                    label_set.add(label)
    ...
    def infer_force_root_constituent(trees):
        for tree in trees:
            for spans in get_labeled_spans(tree):
                if spans != []:
                    if spans[0]:
                        if not spans[0][-1]:
                            return False
    ...
    def chart_from_tree(self, tree):
        ...
        num_words = len(tree.leaves())
        left_chart = np.full((num_words, num_words), -100, dtype=int)
        # e.g. array([[-100, -100, -100],
        #           [-100, -100, -100],
        #           [-100, -100, -100]])
        left_chart = np.tril(left_chart, -1)
        # array([[   0,    0,    0],
        #         [-100,    0,    0],
        #         [-100, -100,    0]])

        # Now all invalid entries are filled with -100, and valid entries with 0
        for start, end, label in left_span:
            # Previously unseen unary chains can occur in the dev/test sets.
            # For now, we ignore them and don't mark the corresponding chart
            # entry as a constituent.
            if label in self.label_vocab:
                left_chart[start, end] = self.label_vocab[label]

        num_words = len(tree.leaves())
        right_chart = np.full((num_words, num_words), -100, dtype=int)
        # e.g. array([[-100, -100, -100],
        #           [-100, -100, -100],
        #           [-100, -100, -100]])
        right_chart = np.tril(right_chart, -1)
        # array([[   0,    0,    0],
        #         [-100,    0,    0],
        #         [-100, -100,    0]])

        # Now all invalid entries are filled with -100, and valid entries with 0
        for start, end, label in right_span:
            # Previously unseen unary chains can occur in the dev/test sets.
            # For now, we ignore them and don't mark the corresponding chart
            # entry as a constituent.
            if label in self.label_vocab:
                right_chart[start, end] = self.label_vocab[label]

        return chart, left_chart, right_chart
    ...
    def charts_from_pytorch_scores_batched(self, scores, lengths):
        ...
        # dist_index = 4
        # ############
        # topk = dist.topk(tmp_k)
        # amax = topk[dist_index]
...
class SpanClassificationMarginLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        # gold_event has the same shape as logits does: [b, s, s, label_vocab_size]
