...
from analysis.get_pattern_constituent_pair import get_frequent_patterns, load_compatitble_data, get_multi_ngram_pattern_children
from analysis.trees import load_trees as outdated_load_trees
...
def make_hparams():
    return nkutil.HParams(
        ...
        pattern_ratio_threshold = -1.0,
        pattern_num_threshold = -1,
        total_num_threshold=-1,
        pattern_loss_scale=1.0,
        pattern_num_negative=5,
        # pattern-constituent consistency task
        use_compatible=False,
        compatible_data_path="",
        compatible_num_threshold=0,
        compatible_loss_scale=1.0,
        compatible_num_negative=5,
        # sibling-classification
        use_sibling=False,
        sibling_loss_scale=1.0,
        # sibling compatible
        use_sibling_compatible=False,
        sibling_compatible_loss_scale=1.0,
        sibling_compatible_path=" ",
        sibling_compatible_threshold=1,
        num_negative_sibling_compatible=5,
...
def run_train(args, hparams):
    if args.numpy_seed is not None:
        ...
        torch.cuda.manual_seed(args.numpy_seed)
    ...
    if hparams.use_pattern or hparams.use_compatible:
        # pattern classification task
        strip_spmrl_features = True
        if args.text_processing == 'chinese':
            strip_spmrl_features = False
        outdated_train_trees = outdated_load_trees(args.train_path, strip_spmrl_features=strip_spmrl_features)

        # pattern_vocab
        frequent_pattern_labels, train_patterns = get_frequent_patterns([outdated_train_trees], n=hparams.num_ngram, pattern_ratio_threshold=hparams.pattern_ratio_threshold, pattern_num_threshold=hparams.pattern_num_threshold, total_num_threshold=hparams.total_num_threshold)
        pattern_list = [" "] + frequent_pattern_labels
        pattern_vocab = {label: i for i, label in enumerate(pattern_list)}
    else:
        pattern_vocab = None

    print("label_vocab size: {}".format(len(label_vocab)))
    if pattern_vocab is not None:
        print("pattern_vocab size: {}".format(len(pattern_vocab)))
    
    if hparams.use_compatible:
        if hparams.compatible_data_path is not "":
            print("Loading pattern_children data from {}".format(hparams.compatible_data_path))
            _pattern_children = load_compatitble_data(hparams.compatible_data_path)
        else:
            print("Cannot find a pattern_children data path. Generating the data...")
            _pattern_children = get_multi_ngram_pattern_children(outdated_train_trees, num_ngram=hparams.num_ngram)
        if hparams.compatible_num_threshold > 0:
            pattern_children = dict()
            for key, value in _pattern_children.items():
                pattern_children[key] = dict()
                for label, num in value.items():
                    if label == "[SELF]":
                        continue
                    if num >= hparams.compatible_num_threshold:
                        pattern_children[key][label] = num
        else:
            pattern_children = _pattern_children
    else:
        pattern_children = None
    
    if hparams.use_sibling:
        if not os.path.exists(hparams.sibling_compatible_path):
            print("Cannot find a sibling data path. Generating the data...")
            from analysis.get_sibling import ParentedTree, get_labeled_spans
            import json
            ...
            with open(hparams.sibling_compatible_path, 'w', encoding='utf-8') as f:
    ...
    strip_top = True
    if 'Hebrew' in args.train_path or 'Hungarian' in args.train_path or 'Arabic' in args.train_path:
        strip_top = False
    if "German" in args.train_path:
        strip_top = False
    data_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=functools.partial(
            ...
            subbatch_max_tokens=args.subbatch_max_tokens,
            get_pattern_function = get_frequent_patterns,
            strip_top = strip_top,
        ),
    )

    if hparams.use_pattern:
        detailed_loss_terms = ["pattern_loss", "compatible_loss"]
    elif hparams.use_sibling:
        detailed_loss_terms = ["sibling_loss", "sibling_compatible_loss"]
    elif hparams.use_regularization:
        detailed_loss_terms = ["reg_loss", "pattern_loss"]
    else:
        detailed_loss_terms = ["reg_loss", "pattern_loss"]
    ...
    for epoch in itertools.count(start=1):
        ...
        for batch_num, batch in enumerate(data_loader, start=1):
            ...
            batch_detailed_loss_value = dict()
            return_confusion_matrix = True
            for subbatch_size, subbatch in batch:
                loss, detailed_loss, _confusion_matrix = parser.compute_loss(subbatch, return_confusion_matrix=return_confusion_matrix)
                # confusion matrix
                ...
                for key, value in detailed_loss.items():
                    if key in batch_detailed_loss_value:
                        batch_detailed_loss_value[key] += value if value != None else 0.0
                    else:
                        batch_detailed_loss_value[key] = value if value != None else 0.0
            ...
            print(
                ...
                "total-elapsed {}".format(
                    ...
                    total_processed,
                    batch_loss_value,
                    detailed_loss_terms[0],
                    batch_detailed_loss_value.get(detailed_loss_terms[0], 0.0),
                    detailed_loss_terms[1], 
                    batch_detailed_loss_value.get(detailed_loss_terms[1], 0.0),
                    grad_norm,
                    format_elapsed(epoch_start_time),
                    format_elapsed(start_time),
                )
            )
            if type(confusion_matrix) != type(None):
...
def run_test(args):
    ...
    test_treebank = treebanks.load_trees(
        test_path, args.test_path_text, args.text_processing
    )
    ...
    parser = parse_chart.ChartParser.from_trained(model_path, input_pretrained_model_path=args.pretrained_model_path)
...
def main():
    ...
    subparser.add_argument("--numpy-seed", type=int, default='42')
    ...
    subparser.add_argument("--pretrained-model-path", required=True)
